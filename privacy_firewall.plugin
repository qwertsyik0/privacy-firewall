...
__description__ = (
    "Комплексная локальная защита исходящих данных. Проверяет текст, "
    "получателя и вложения, позволяет выборочно маскировать найденные "
    "данные, распознаёт ФИО и документы, проверяет содержимое PDF, "
    "Office и текстовых файлов, скрывает GPS-координаты, ссылки на карты, "
    "адреса, улицы, дома, квартиры, города, сёла и другие населённые пункты. "
    "Изображения не анализируются и не изменяются."
)
